  ...
  return (
    <Card className="w-full max-w-md h-[650px] flex flex-col shadow-2xl border-none rounded-2xl overflow-hidden bg-white">
          ...
          <div className="py-6 space-y-6">
            {messages.map((m, i) => (
              ...
              >
                ...
                <div
                  className={`p-3.5 rounded-2xl text-[13.5px] leading-relaxed shadow-sm max-w-[85%] ${
                    m.sender === "user"
            ...
            {isLoading && (
              <div className="flex gap-3 flex-row">
                ...
                <div className="bg-white border border-slate-100 p-3 rounded-2xl rounded-tl-none text-xs text-slate-400 font-medium">
                  Lisa is typing...
                </div>
              </div>
            )}
      ...
      <CardFooter className="p-4 border-t bg-white shrink-0">
        <div className="flex w-full gap-2 items-center bg-slate-100 rounded-full px-4 py-1 border border-slate-200 focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-400 transition-all">
          <Input
            value={input}
            ...
            disabled={isLoading}
            className="flex-1 bg-transparent border-none focus-visible:ring-0 focus-visible:ring-offset-0 text-sm py-6"
          />
          <Button
            onClick={handleSend}
            disabled={isLoading || !input.trim()} // Requirement [cite: 31]
            size="icon"
            className="rounded-full w-8 h-8 bg-blue-600 hover:bg-blue-700 shrink-0"
          >
